...
contract InternalAccount is BaseAccount, InternalAccountEvents {
    ...
    function _preStrategyLiquidation(address recipient) internal view returns (uint256 amountBefore) {
        // Track the amount liquidate by checking the actual asset balance of the liquidator before and after
        amountBefore = actualAsset.balanceOf(recipient);
    }
    ...
    function _postStrategyLiquidation(
        ...
        internal
    {
        if (actualAsset.balanceOf(address(recipient)) < (expectedReceived + amountBefore)) {
