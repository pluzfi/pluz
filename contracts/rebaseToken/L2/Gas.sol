...
import { SafeTransferLib } from "../libraries/solmate/src/utils/SafeTransferLib.sol";
...
contract Gas is IGas, Initializable, Semver {
    ...
    // Pluz.sol --> controls all dAPP accesses to Gas.sol
    address public immutable pluzConfigurationContract;

    // BaseFeeVault.sol -> fees from gas claims directed here
    address public immutable pluzFeeVault;
    ...
    /**
     * @notice Constructs the pluz gas contract.
     * @param _admin The address of the admin.
     * @param _pluzConfigurationContract The address of the Pluz configuration contract.
     * @param _pluzFeeVault The address of the Pluz fee vault.
    */
    constructor (
        address _admin,
        address _pluzConfigurationContract,
        address _pluzFeeVault
    ) Semver(1, 0, 0) {
        admin =  _admin;
        pluzConfigurationContract = _pluzConfigurationContract;
        pluzFeeVault = _pluzFeeVault;
    ...
    /**
     * @notice Allows only the Pluz Configuration Contract to call a function
     */
    modifier onlyPluzConfigurationContract() {
        require(msg.sender == pluzConfigurationContract, "Caller must be pluz configuration contract");
        _;
    }
    ...
    function adminClaimGas(address contractAddress) external onlyAdmin returns (uint256) {
        ...
        SafeTransferLib.safeTransferETH(pluzFeeVault, etherBalance);
        return etherBalance;
    }
    /**
     * @notice Allows an authorized user to set the gas mode for a contract via the PluzConfigurationContract
     * @param contractAddress The address of the contract
     * @param mode The new gas mode for the contract
     */
    function setGasMode(address contractAddress, GasMode mode) external onlyPluzConfigurationContract {
        // retrieve gas params
    ...
    function claim(address contractAddress, address recipientOfGas, uint256 gasToClaim, uint256 gasSecondsToConsume) public onlyPluzConfigurationContract() returns (uint256)  {
        // retrieve gas params
        ...
        if (penalty > 0) {
            SafeTransferLib.safeTransferETH(pluzFeeVault, penalty);
        }
